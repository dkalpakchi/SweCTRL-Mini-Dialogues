...
def combine_generated_parts(parts, tok):
    total_text = ""
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', type=str, default="ctrl_minip_ddp_256_v4/checkpoint-2786844", help="Checkpoint path")
    parser.add_argument('-n', '--number', type=int, default=1, help="Number of samples per prompts")
    ...
    parser.add_argument('-caf', '--ctrl-args-file', type=str, default="ctrl_minip_ddp_256_v4/ctrl_args.bin", help="CTRL settings file")
    ...
    parser.add_argument('-npf', '--no-prompt-file', action='store_true', help="Disable reading the prompt file")
    args = parser.parse_args()

    if args.no_prompt_file:
        predef_prompts = {}
    elif os.path.exists("prompts.yaml"):
        with open("prompts.yaml") as f:
            predef_prompts = yaml.load(f, Loader=yaml.FullLoader)
    ...
    prefix = ""
    if args.control:
        prefix = START_C_CODES[args.control]

    prompts = predef_prompts.get(args.control.split("/")[0], [])
    if args.prompt:
    ...
    for prompt in prompts:
        for _ in range(args.number):
            ...
            LIMITS = {
                ...
                'default': ctrl_args.sequence_length,
            }
            ...
            if total_length > max_len:
                ...
                steps = math.ceil(left_to_generate / ws) - 1
                ...
                for _ in tqdm(range(steps)):
                    ...
                    total_generated += min(output.shape[1] - ws, max_new_tokens) 
                print("In total: {} tokens.".format(total_generated))

                generated_texts.append({
                    "text": combine_generated_parts(parts, tokenizer),
    ...
    fname_prefix = args.control.replace("/", "_")
    with jsl.open('{}_{}.jsonl'.format(fname_prefix, int(time.time())), mode='w') as writer:
